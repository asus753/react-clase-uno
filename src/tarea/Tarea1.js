...
export function Tarjeta(props) {
  return (
    <div className='tarjeta'>
      <img src={props.imagen} alt='Foto de perfil' className='tarjeta-img'></img>
  ...
  return (
    <article className="post">
      <header className="post-header">
        <h2 className="post-title">{props.tituloBlog}</h2>
        <Tarjeta nombre={props.autor.nombre} titulo={props.autor.titulo} imagen={props.autor.imagen} />
      </header>
      {props.parrafos.split('\n').map((parrafo,i) => {    //revisasr - No agrega los parrafos al div
        return <p className="post-paragraph" key={i}>{parrafo}</p>
      })}
    </article>
  );
}
